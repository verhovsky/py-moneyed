...
ANG = add_currency('ANG', '532', 'Netherlands Antillean Guilder', ['CURAÇAO', 'SINT MAARTEN (DUTCH PART)'])
...
AUD = add_currency('AUD', '036', 'Australian Dollar', ['AUSTRALIA', 'CHRISTMAS ISLAND',
                                                       ...
                                                       'TUVALU'])
AWG = add_currency('AWG', '533', 'Aruban Florin', ['ARUBA'])
AZN = add_currency('AZN', '944', 'Azerbaijan Manat', ['AZERBAIJAN'])
BAM = add_currency('BAM', '977', 'Convertible Mark', ['BOSNIA AND HERZEGOVINA'])
...
BTN = add_currency('BTN', '064', 'Ngultrum', ['BHUTAN'])
BWP = add_currency('BWP', '072', 'Pula', ['BOTSWANA'])
BYN = add_currency('BYN', '933', 'Belarusian Ruble', ['BELARUS'])
BZD = add_currency('BZD', '084', 'Belize Dollar', ['BELIZE'])
CAD = add_currency('CAD', '124', 'Canadian Dollar', ['CANADA'])
CDF = add_currency('CDF', '976', 'Congolese Franc', ['DEMOCRATIC REPUBLIC OF THE CONGO'])
CHE = add_currency('CHE', '947', 'WIR Euro', ['SWITZERLAND'])
CHF = add_currency('CHF', '756', 'Swiss Franc', ['LIECHTENSTEIN', 'SWITZERLAND'])
...
CUC = add_currency('CUC', '931', 'Peso Convertible', ['CUBA'])
CUP = add_currency('CUP', '192', 'Cuban Peso', ['CUBA'])
CVE = add_currency('CVE', '132', 'Cabo Verde Escudo', ['CABO VERDE'])
CZK = add_currency('CZK', '203', 'Czech Koruna', ['CZECHIA'])
...
EUR = add_currency('EUR', '978', 'Euro', ['ÅLAND ISLANDS', 'ANDORRA', 'AUSTRIA', 'BELGIUM',
                                          'CYPRUS', 'ESTONIA', 'FINLAND', 'FRANCE', 'FRENCH GUIANA',
                                          'FRENCH SOUTHERN TERRITORIES', 'GERMANY', 'GREECE',
                                          'GUADELOUPE', 'IRELAND', 'ITALY', 'LATVIA', 'LITHUANIA',
...
GBP = add_currency('GBP', '826', 'Pound Sterling', ['GUERNSEY', 'ISLE OF MAN', 'JERSEY', 'UNITED KINGDOM'])
...
GNF = add_currency('GNF', '324', 'Guinean Franc', ['GUINEA'])
...
HRK = add_currency('HRK', '191', 'Kuna', ['CROATIA'])
HTG = add_currency('HTG', '332', 'Gourde', ['HAITI'])
...
INR = add_currency('INR', '356', 'Indian Rupee', ['BHUTAN', 'INDIA'])
...
KMF = add_currency('KMF', '174', 'Comorian Franc ', ['COMOROS'])
KPW = add_currency('KPW', '408', 'North Korean Won', ['NORTH KOREA'])
KRW = add_currency('KRW', '410', 'Won', ['SOUTH KOREA'])
...
LAK = add_currency('LAK', '418', 'Lao Kip', ['LAO PEOPLE’S DEMOCRATIC REPUBLIC'])
...
LSL = add_currency('LSL', '426', 'Loti', ['LESOTHO'])
LYD = add_currency('LYD', '434', 'Libyan Dinar', ['LIBYA'])
...
MZN = add_currency('MZN', '943', 'Mozambique Metical', ['MOZAMBIQUE'])
NAD = add_currency('NAD', '516', 'Namibia Dollar', ['NAMIBIA'])
...
PEN = add_currency('PEN', '604', 'Sol', ['PERU'])
...
RON = add_currency('RON', '946', 'Romanian Leu', ['ROMANIA'])
...
TOP = add_currency('TOP', '776', 'Pa’anga', ['TONGA'])
...
USD = add_currency('USD', '840', 'US Dollar', ['AMERICAN SAMOA', 'BONAIRE, SINT EUSTATIUS AND SABA',
                                               'BRITISH INDIAN OCEAN TERRITORY',
                                               'ECUADOR', 'EL SALVADOR', 'GUAM', 'HAITI', 'MARSHALL ISLANDS',
                                               'MICRONESIA',
                                               'NORTHERN MARIANA ISLANDS', 'PALAU', 'PANAMA', 'PUERTO RICO',
                                               'TIMOR-LESTE', 'TURKS AND CAICOS ISLANDS', 'UNITED STATES',
                                               'UNITED STATES MINOR OUTLYING ISLANDS',
                                               'VIRGIN ISLANDS (BRITISH)', 'VIRGIN ISLANDS (U.S.)'])
USN = add_currency('USN', '997', 'US Dollar (Next day)', ['UNITED STATES'])
UYI = add_currency('UYI', '940', 'Uruguay Peso en Unidades Indexadas (UI)', ['URUGUAY'])
UYU = add_currency('UYU', '858', 'Peso Uruguayo', ['URUGUAY'])
UYW = add_currency('UYW', '927', 'Unidad Previsional', ['URUGUAY'])
UZS = add_currency('UZS', '860', 'Uzbekistan Sum', ['UZBEKISTAN'])
VES = add_currency('VES', '928', 'Bolívar Soberano', ['VENEZUELA'])
...
XBA = add_currency('XBA', '955', 'Bond Markets Unit European Composite Unit (EURCO)', [])
XBB = add_currency('XBB', '956', 'Bond Markets Unit European Monetary Unit (E.M.U.-6)', [])
XBC = add_currency('XBC', '957', 'Bond Markets Unit European Unit of Account 9 (E.U.A.-9)', [])
XBD = add_currency('XBD', '958', 'Bond Markets Unit European Unit of Account 17 (E.U.A.-17)', [])
...
XOF = add_currency('XOF', '952', 'CFA Franc BCEAO', ['BENIN', 'BURKINA FASO', "CÔTE D'IVOIRE",
...
XXX = add_currency('XXX', '999', 'The codes assigned for transactions where no currency is involved', [])
YER = add_currency('YER', '886', 'Yemeni Rial', ['YEMEN'])
ZAR = add_currency('ZAR', '710', 'Rand', ['LESOTHO', 'NAMIBIA', 'SOUTH AFRICA'])
ZMW = add_currency('ZMW', '967', 'Zambian Kwacha', ['ZAMBIA'])
ZWL = add_currency('ZWL', '932', 'Zimbabwe Dollar', ['ZIMBABWE'])
